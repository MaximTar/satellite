...
//        double b = p / Math.sqrt(1 - e * e);
        ...
        double M = n * (0 - tau);
        double E = KP_solveKepler(M, e, 1E-10);
...
//                (1 + e * NumberUtils.round(Math.cos(v), 3)) * NumberUtils.round(Math.cos(u), 3) * NumberUtils.round(Math.sin(I), 3));
        ...
        ArrayList<Double> Res_list = new ArrayList<>();
        Res_list.add(x);
        Res_list.add(y);
        Res_list.add(z);
        Res_list.add(Vx);
...
//        double v1 = Math.asin(vr * p / (c * e));
        ...
        } else if (sinv == 0 && cosv == 1) {
            v = 0;
        } else if (sinv == 1 && cosv == 0) {
            v = Math.PI / 2;
        } else if (sinv == 0 && cosv == -1) {
            v = Math.PI;
        } else if (sinv == -1 && cosv == 0) {
...
//        double i = Math.acos(c3 / c);
        ...
        ArrayList<Double> Res_list = new ArrayList<>();
        Res_list.add(Math.toDegrees(omega));
        Res_list.add(Math.toDegrees(i));
        Res_list.add(Math.toDegrees(w));
        Res_list.add(p);
        Res_list.add(e);
        Res_list.add(tau);
        return Res_list;
...
//    public static double newton_method(double e, double mu, double p, double acc) {
//        double x1 = -1;
//        double x2 = 2;
//        double diff;
//        diff = Math.abs(x1 - x2);
...
//    }
    ...
    public static double KP_solveKepler(double M, double e, double acc) {
        double curE, nextE;
        nextE = M;
        do {
            curE = nextE;
