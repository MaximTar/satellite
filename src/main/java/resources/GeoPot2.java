...
//    }
//
//    public static GeoPot sum(GeoPot area, GeoPot b) {
...
////    }
//
//    public static GeoPot multV(double area, GeoPot b) {
//        GeoPot c = new GeoPot(0, 0, 0);
//        c.x = area * b.x;
//        c.y = area * b.y;
//        c.z = area * b.z;
...
//    }
//
//    public static GeoPot multV(GeoPot b, double area) {
//        GeoPot c = new GeoPot(0, 0, 0);
//        c.x = area * b.x;
//        c.y = area * b.y;
//        c.z = area * b.z;
...
//    }
//
//    public static double norm(GeoPot area) {
//        return 1 / Math.sqrt((area.x * area.x) + (area.y * area.y) + (area.z * area.z));
//    }
//
//    public static GeoPot rev(GeoPot area) {
//        return new GeoPot(-area.x, -area.y, -area.z);
//    }
//
...
//        GeoPot XYZoverR = multV(rECEF, rs);
//        double rr = rs * rs;
//
...
//        GeoPot DR = multV(rev(XYZoverR), rr);
...
//
//        double[] RL = new double[degree + 1];
//        double[] PL = new double[degree + 1];
//        double[] UL = new double[degree + 1];
//
//        RL[0] = rGeoFM * rGeoR0;
//        RL[1] = rGeoR0 * RL[0];
//        PL[0] = XYZoverR.z;  // P_1(x)=1 Legendre polynomial m=0
//        PL[1] = 1.5 * XYZoverR.z * XYZoverR.z - 0.5; // P_2(x) = 3 / 2 * x * x - 1 / 2 Legendre polynomial
//        UL[0] = 1; // derivative P_1'(x)=1 m=0
//        UL[1] = 3 * XYZoverR.z; // derivative P_2'(x)=3*x
//
//        for (int n = 2; n <= degree; n++) {
//            RL[n] = rGeoR0 * RL[n - 1];
//            PL[n] = ((2 * n - 1) / n) * XYZoverR.z * PL[n - 1] - ((n - 1) / n) * PL[n - 2]; // P_n(x)
...
//
//        GeoPot egECEF = multV(EarthGravity, DR);
//
...
//                }
//
//                System.arraycopy(UL, 0, PL, 0, degree + 1);
//                for (int n = 0; n <= qy - 2; n++) {
//                    UL[n] = 0;
//                }
//                for (int n = qy - 1; n <= degree; n++) {
//                    UL[n] = (2 * n - 1) * PL[n - 1];
//
//                    if (n > (qy + 1)) { // in ul[n] are d(p_(nm)(z / r))/d(z / r)
//                        UL[n] = UL[n] + UL[n - 2];
//                    }
//                }
//
...
//            }
//
//            for (int n = qy - 1; n <= degree - 1; n++) {
//                // for (fm / r)*(r_0 / r) ^ n
//                GeoPot U = multV((n + 1) * RL[n - 1], multV(EarthEquatorialRadius, DR)); // d((fm / r) * (r_0 / r) ^ n) / d(1 / r) * d(1 / r) / dr
//                // p_(nm)->d ^ m(P_n(z / r)) / d(z / r) ^ m
//                GeoPot V = multV(UL[n], DZ); // d(p_(nm) (z / r))/d(z / r) * d(z / r) / (d x, y, z)
...
//                GeoPot W = sum(multV((c * xax + s * yax), DX), multV((c * xay + s * yay), DY));
//
//                egECEF = sum(sum(egECEF, multV(multV(U, PL[n]), (c * xam + s * yam))), sum(multV(multV(RL[n], V), (c * xam + s * yam)), multV(RL[n] * PL[n], W))); // current n, m force
//            }
//
//            if (m == 1) {
//                nhcs = 0;
